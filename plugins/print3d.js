function ShowErrorDialog ()
{
    return OV.UI.ShowMessageDialog (
        OV.UI.Loc ('3D Print Failed'),
        OV.UI.Loc ('Failed to send the model. Please try again later.'),
        null
    );
}
...
function SendModelFor3DPrinting (model)
{
    ...
    progressDialog.Init (OV.UI.Loc ('Uploading Model'));
    progressDialog.Open ();
...
OV.RegisterToolbarPlugin ({
    registerButtons : (pluginInterface) => {
        pluginInterface.createButton ('print3d', OV.UI.Loc ('3D print'), ['only_full_width', 'only_on_model'], () => {
            let dialog = new OV.UI.ButtonDialog ();
            let contentDiv = dialog.Init (OV.UI.Loc ('3D print'), [
                {
                    name : OV.UI.Loc ('Cancel'),
                    subClass : 'outline',
                    onClick () {
                        dialog.Close ();
                    }
            ...
            OV.Engine.AddDiv (contentDiv, 'ov_dialog_section', OV.UI.Loc ('Your model will be converted to STL and uploaded to <a href="https://www.pcbway.com" target="_blank">PCBWay</a> to finish the 3D printing process.'));
